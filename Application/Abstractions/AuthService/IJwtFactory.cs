...
public interface IJwtFactory
{
    ...
    string Create(Guid userId, string email , string userName, IEnumerable<Claim>? extraClaims = null);
    /// <summary>Optionally issues a new refresh-token too.</summary>
    (string AccessToken, string RefreshToken) CreateWithRefresh(
        Guid userId, string? email , string userName, IEnumerable<string>? roles = null);
}
